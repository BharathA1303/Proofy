"""
backend/app/services/documents/national_id/national_id_validator.py

Module 2: National ID Document Validation Service.
Focused on the Indian National ID Reference Profile.

Validates:
1. Required field presence (docNumber / identityNumber, name, and dob or yearOfBirth).
...
- A checksum failure is evidence of identifier inconsistency.
  ...
  (Severity is warning / format warning, never "forged").
...
def validate_national_id_document(
    traveler: Optional[TravelerFields],
    reference_date: Optional[datetime.date] = None,
) -> Dict[str, Any]:
    """
    Execute structural validation checks on extracted National ID data.
    ...
    if not traveler:
        return {
            ...
            "summary": "No extracted traveler fields available for National ID validation.",
            "checks": {
    ...
    if missing_fields:
        ...
        req_valid = True
        req_status = "passed"
        req_msg = "Required National ID fields are present."
    ...
    norm_id, norm_err = normalize_national_id(raw_id)
    masked_display = mask_national_id(norm_id or raw_id)

    if norm_err == "AMBIGUOUS_IDENTIFIER":
        id_fmt_valid = False
        id_fmt_status = "warning"
        id_fmt_msg = f"Ambiguous characters detected in National ID identifier '{masked_display}'."
        issues.append({
            "severity": "warning",
            "check": "identifier_format",
            "message": id_fmt_msg,
        })
    elif not norm_id or len(norm_id) != 12:
        id_fmt_valid = False
        id_fmt_status = "failed"
        id_fmt_msg = f"National ID identifier '{masked_display}' is invalid (expected 12 numeric digits)."
        issues.append({
            "severity": "critical",
            "check": "identifier_format",
            "message": id_fmt_msg,
        ...
        id_fmt_msg = f"National ID format valid (12-digit standard, {masked_display})."

    checks["identifier_format"] = {
        "status": id_fmt_status,
        "valid": id_fmt_valid,
        "masked_identifier": masked_display,
        "message": id_fmt_msg,
    }
    ...
    if norm_id and len(norm_id) == 12:
        ...
        if chk_res == ChecksumStatus.PASS:
            chk_valid = True
            chk_status = "passed"
            chk_msg = f"Verhoeff checksum passed for identifier ({masked_display})."
        elif chk_res == ChecksumStatus.FAIL:
            chk_valid = False
            chk_status = "warning"
            # Crucial: Checksum failure is evidence of identifier inconsistency, NOT proof of forgery!
            chk_msg = f"Identifier validation failed: Verhoeff checksum mismatch on {masked_display}."
            issues.append({
                "severity": "warning",
                "check": "identifier_checksum",
                "message": chk_msg,
            })
        else:
            chk_valid = False
            chk_status = "inconclusive"
            chk_msg = f"Verhoeff checksum inconclusive on {masked_display}."
    else:
        chk_res = ChecksumStatus.INCONCLUSIVE
        chk_valid = False
        chk_status = "unknown"
        chk_msg = "Checksum not verifiable without a 12-digit numeric identifier."

    checks["identifier_checksum"] = {
        "status": chk_status,
        "valid": chk_valid,
        "checksum_status": chk_res.value,
        "message": chk_msg,
    }
    ...
    if extracted_yob:
        ...
        if extracted_yob < 1900:
            ...
            if date_info["date_of_birth"]:
                date_msg = f"Date of birth verified: {date_info['date_of_birth']}."
            else:
                date_msg = f"Year of birth verified: {extracted_yob} (year-only profile)."
    else:
        date_valid = False
        date_status = "warning"
    ...
    checks["date_validity"] = {
        "status": date_status,
        "valid": date_valid,
        "date_of_birth": date_info.get("date_of_birth"),
        "year_of_birth": extracted_yob,
        "is_year_only": date_info.get("is_year_only", False),
        "message": date_msg,
    }

    # ── Check 5: QR / Barcode Field Consistency (where applicable) ───────────
    qr_payload = traveler.qrPayload
    if qr_payload:
        parsed_qr = parse_national_id_qr_payload(qr_payload)
        ocr_comparison_fields = {
            "identity_number": norm_id,
            "name": name,
            "dob": date_info.get("date_of_birth") or str(extracted_yob),
        }
    ...
    if any(issue["severity"] == "critical" for issue in issues):
        ...
        summary = f"National ID validation failed: {issues[0]['message']}"
    elif any(issue["severity"] in ("high", "warning") for issue in issues):
        overall_status = "warning"
        summary = f"National ID validated with warnings: {issues[0]['message']}"
    elif not req_valid or not id_fmt_valid:
        overall_status = "failed"
        summary = "National ID failed structural validation checks."
    else:
        overall_status = "passed"
        summary = "All National ID structural, format, and checksum checks passed."
