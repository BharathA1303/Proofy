"""
backend/app/services/face/minifasnet_model.py

PyTorch implementation of the MiniFASNet architecture for Presentation Attack Detection (PAD).
Ref: Minivision Silent-Face-Anti-Spoofing.

Complies with ISO/IEC 30107-3 presentation-attack detection concepts.
Evaluates bona fide human biometric presentation vs. presentation attacks
(printed photographs, digital screen replays, 2D masks).
"""
...
class Residual(Module):
    def __init__(self, c1, c2, c3, num_block, groups, kernel=(3, 3), stride=(1, 1), padding=(1, 1)):
        ...
        for i in range(num_block):
            modules.append(
                DepthWise(c1[i], c2[i], c3[i], residual=True, kernel=kernel, padding=padding, stride=stride, groups=groups)
            )
...
class DepthWiseSE(Module):
    def __init__(self, c1, c2, c3, residual=False, kernel=(3, 3), stride=(2, 2), padding=(1, 1), groups=1, se_reduc=4):
        super().__init__()
        c1_in, c1_out = c1
        c2_in, c2_out = c2
        c3_in, c3_out = c3
        ...
        self.project = LinearBlock(c3_in, c3_out, kernel=(1, 1), padding=(0, 0), stride=(1, 1))
        self.residual = residual
        self.se_module = SEModule(c3_out, se_reduc)
...
class ResidualSE(Module):
    def __init__(self, c1, c2, c3, num_block, groups, kernel=(3, 3), stride=(1, 1), padding=(1, 1), se_reduc=4):
        super().__init__()
        modules = []
        for i in range(num_block):
            modules.append(
                DepthWiseSE(c1[i], c2[i], c3[i], residual=True, kernel=kernel, padding=padding, stride=stride, groups=groups, se_reduc=se_reduc)
            )
        self.model = Sequential(*modules)
...
class MiniFASNet(Module):
    def __init__(self, keep: list, embedding_size: int = 128, conv6_kernel=(5, 5), drop_p: float = 0.75, num_classes: int = 3, img_channel: int = 3):
        super().__init__()
        ...
        self.conv_6_flatten = Sequential(AdaptiveAvgPool2d(1))

        self.linear = Linear(keep[48], embedding_size, bias=False)
...
# Architecture Channel Configurations
_MINI_FAS_NET_V2_CHANNELS = [
    32, 32, 103, 103, 64, 13, 13, 64, 26, 26, 64, 13, 13, 64, 52, 52, 64,
    231, 231, 128, 154, 154, 128, 52, 52, 128, 26, 26, 128, 52, 52, 128,
    26, 26, 128, 26, 26, 128, 308, 308, 128, 26, 26, 128, 26, 26, 128,
    512, 512
]

_MINI_FAS_NET_V1_CHANNELS = [
...
def MiniFASNetV2(embedding_size=128, conv6_kernel=(5, 5), num_classes=3) -> MiniFASNet:
    return MiniFASNet(_MINI_FAS_NET_V1_CHANNELS, embedding_size=embedding_size, conv6_kernel=conv6_kernel, num_classes=num_classes)


def MiniFASNetV1SE(embedding_size=128, conv6_kernel=(5, 5), num_classes=3) -> MiniFASNetSE:
    return MiniFASNetSE(_MINI_FAS_NET_V1_CHANNELS, embedding_size=embedding_size, conv6_kernel=conv6_kernel, num_classes=num_classes)
