"""
backend/app/services/registry/providers/mock_pan_card.py

Development Mock Registry Provider for PAN Cards.
Emulates an Income Tax Department / NSDL PAN database lookup.

DEVELOPMENT USE ONLY — Not a real ITD/NSDL database.
"""
...
class _PanCardRecordProxy(dict):
    def __getitem__(self, key: str):
        res = government_registry_db.lookup_document("pan_card", key)
        if not res:
            raise KeyError(key)
        return res
    ...
    def __contains__(self, key: object):
        if not isinstance(key, str):
            return False
        return government_registry_db.lookup_document("pan_card", key) is not None
    ...
    def keys(self):
        return government_registry_db.get_all_records_for_type("pan_card").keys()


_MOCK_PAN_RECORDS = _PanCardRecordProxy()


class MockPanCardRegistryProvider(RegistryProvider):
    """Mock PAN Card registry provider for development and testing."""
    _PROVIDER_ID     = "mock_pan_card_registry"
    _SUPPORTED_TYPES = {"pan_card", "pancard", "pan"}
    ...
    def initialize(self) -> None:
        logger.info("MockPanCardRegistryProvider initialized: %s (DEVELOPMENT MOCK)", self._PROVIDER_ID)
    ...
    def health_check(self) -> RegistryProviderStatus:
        return RegistryProviderStatus(
            ...
            details="Development mock provider for PAN Card. NOT a real NSDL/ITD database.",
        )
    ...
    def verify(self, request: RegistryVerificationRequest) -> RegistryVerificationResponse:
        ...
        logger.info("MockPanCardRegistry lookup: id=%s pan=%s", request.verification_id, lookup_key[:3] + "XXXXXXX" if len(lookup_key) >= 3 else lookup_key)

        if not lookup_key or lookup_key not in _MOCK_PAN_RECORDS:
            return self._build_response(
                ...
                evidence=[RegistryEvidence(
                    ...
                    description=f"No development mock record found for PAN Card '{lookup_key[:3]}XXXXXXX'.",
                )],
                response_time_ms=t_elapsed_ms,
            )

        raw_record = _MOCK_PAN_RECORDS[lookup_key]
        ...
        evidence = _build_pan_evidence(status, field_results, lookup_key)
        if raw_record.get("is_blacklisted"):
            evidence.insert(0, RegistryEvidence(
                type="watchlist_hit", severity="critical",
                description=f"INCOME TAX WATCHLIST ALERT: {raw_record.get('watchlist_reason', 'PAN Card suspended/revoked.')}",
            ))
    ...
    def _build_response(self, request, registry_status, record, field_results, evidence, response_time_ms):
        return RegistryVerificationResponse(
            ...
            audit={
                ...
                "record_found": record is not None,
                "response_time_ms": round(response_time_ms, 2),
                "disclaimer": "DEVELOPMENT MOCK DATA — not a real NSDL/ITD PAN registry result.",
            },
        )


def _build_pan_evidence(status, field_results, pan_num):
    masked = pan_num[:3] + "XXXXXXX" if len(pan_num) >= 3 else pan_num
    evidence = []
    if status == RegistryStatus.MATCHED:
        evidence.append(RegistryEvidence(type="registry_record", severity="info",
            description=f"Development sandbox PAN record located for '{masked}' and identity fields matched."))
    elif status == RegistryStatus.NOT_FOUND:
        evidence.append(RegistryEvidence(type="registry_lookup", severity="warning",
            description=f"No development sandbox PAN record found for '{masked}'."))
    elif status == RegistryStatus.REVOKED:
        evidence.append(RegistryEvidence(type="registry_status", severity="critical",
            description=f"Development sandbox reports PAN '{masked}' as REVOKED."))
        ...
        evidence.append(RegistryEvidence(type="registry_mismatch", severity="high",
            description=f"PAN '{masked}' found but field mismatch in: {', '.join(mismatches)}."))
    else:
